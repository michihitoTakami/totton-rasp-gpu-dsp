/**
 * @file auto_negotiation.cpp
 * @brief Auto-negotiation implementation for Issue #12
 */
...
AudioEngine::RateFamily getRateFamily(int sampleRate) {
  if (PcmFormatSet::is44kFamilyRate(static_cast<uint32_t>(sampleRate))) {
    return AudioEngine::RateFamily::RATE_44K;
  }
  if (PcmFormatSet::is48kFamilyRate(static_cast<uint32_t>(sampleRate))) {
    return AudioEngine::RateFamily::RATE_48K;
  }
...
int getTargetRateForFamily(AudioEngine::RateFamily family) {
  if (family == AudioEngine::RateFamily::RATE_44K) {
...
int getBestRateForFamily(AudioEngine::RateFamily family,
  ...
  const auto &rates = (family == AudioEngine::RateFamily::RATE_44K)
...
NegotiatedConfig negotiate(int inputRate,
  ...
  config.inputFamily = AudioEngine::RateFamily::RATE_UNKNOWN;
  config.outputRate = 0;
  ...
  if (targetOutputRate < inputRate) {
    config.errorMessage =
        "Target output rate (" + std::to_string(targetOutputRate) +
  ...
  if (ratio != 1 && ratio != 2 && ratio != 4 && ratio != 8 && ratio != 16) {
    config.errorMessage =
        "Unsupported input rate: " + std::to_string(inputRate) + " Hz (ratio " +
        std::to_string(ratio) + " not in {1, 2, 4, 8, 16})";
    return config;
  }
