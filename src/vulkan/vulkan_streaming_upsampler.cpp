...
bool ExtractJsonUnsigned(const std::string& json, const std::string& key, std::size_t* out) {
    const std::string pattern = "\"" + key + "\"";
    std::size_t pos = json.find(pattern);
    if (pos == std::string::npos) {
        return false;
...
bool VulkanStreamingUpsampler::LoadFilter(const std::string& jsonPath, std::string* errorMessage) {
    FilterConfig config;
...
std::vector<float> VulkanStreamingUpsampler::ProcessBlock(const float* input, std::size_t count) {
    ...
    overlap_.assign(timeBuffer.end() - static_cast<std::ptrdiff_t>(overlapSize), timeBuffer.end());
    return output;
}
...
bool VulkanStreamingUpsampler::LoadFilterConfig(const std::string& jsonPath, FilterConfig* config, std::string* errorMessage) {
...
bool VulkanStreamingUpsampler::LoadCoefficients(const FilterConfig& config, std::string* errorMessage) {
    std::ifstream file(config.coefficientsPath, std::ios::binary);
    if (!file) {
        if (errorMessage) {
            *errorMessage = BuildError("Failed to open coefficients", config.coefficientsPath);
        }
        return false;
    }
    ...
    file.read(reinterpret_cast<char*>(coefficients.data()), static_cast<std::streamsize>(expectedBytes));
