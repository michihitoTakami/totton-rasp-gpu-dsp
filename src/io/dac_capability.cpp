/**
 * @file dac_capability.cpp
 * @brief DAC capability scanning implementation using ALSA
 */
...
Capability scan(const std::string &device) {
  ...
  int err = snd_pcm_open(&pcm, device.c_str(), SND_PCM_STREAM_PLAYBACK,
                         SND_PCM_NONBLOCK);
  if (err < 0) {
  ...
  if (err < 0) {
    cap.errorMessage =
        "Cannot get hardware params: " + std::string(snd_strerror(err));
    snd_pcm_close(pcm);
    return cap;
  }
  ...
  // Test common high-resolution audio sample rates
...
bool isRateSupported(const Capability &cap, int sampleRate) {
  ...
  // If supportedRates is empty, we couldn't determine which rates are actually
  // supported Be conservative and return false (unknown = unsupported)
  if (cap.supportedRates.empty()) {
    return false;
  }
  return std::find(cap.supportedRates.begin(), cap.supportedRates.end(),
                   sampleRate) != cap.supportedRates.end();
}
