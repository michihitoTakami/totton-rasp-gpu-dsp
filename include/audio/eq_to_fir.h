...
std::vector<std::complex<double>> biquadFrequencyResponse(
    const std::vector<double>& frequencies, const BiquadCoeffs& coeffs, double sampleRate);

std::vector<std::complex<double>> computeEqFrequencyResponse(
    const std::vector<double>& frequencies, const EqProfile& profile, double sampleRate);

std::vector<double> generateR2cFftFrequencies(size_t numBins, size_t fullFftSize,
                                              double sampleRate);

std::vector<std::complex<double>> computeEqResponseForFft(size_t filterFftSize,
                                                          size_t fullFftSize,
