/*
 * Shared PCM format constraints for totton-rasp-gpu-dsp.
 * Keeps the single source of truth for accepted sample rates / channels /
 * formats.
 */
#pragma once
...
constexpr std::array<uint32_t, N1 + N2>
concat(const std::array<uint32_t, N1> &a, const std::array<uint32_t, N2> &b) {
...
constexpr std::array<uint32_t, 5> kRates44k = {44100, 88200, 176400, 352800,
                                               705600};
constexpr std::array<uint32_t, 5> kRates48k = {48000, 96000, 192000, 384000,
                                               768000};
...
inline std::vector<uint32_t> allowedSampleRatesVector() {
  return std::vector<uint32_t>(kAllowedSampleRates.begin(),
